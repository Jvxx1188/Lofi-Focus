...
import {Images } from "./selecteds/selected";
...
export default function ImageCase(){
    ...
    return <Style>
        ...
        <div className="flex flex-col overflow-y-scroll p-[0px] gap-1">
            ...
            {
            cartegory.values.map((img)=>{
               return <div className="rounded-xl hover:outline duration-100 hover:cursor-pointer hover:outline-white overflow-hidden active:outline-6 active:outline-black active:duration-0" onClick={()=>bgcontroller.Set.Image(img.ImageUrl)}>
                <img className="aspect-video" src={img.templateurl} alt="" />
                </div>
            })
            }
            </div>
